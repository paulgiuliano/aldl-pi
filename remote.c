...
void *remote_init(void *aldl_in) {
  ...
  while(1) {
    ...
    if(access("/etc/aldl-pi/aldl-stop",F_OK) != -1) {
      exit(1);
    }

    /* if /etc/aldl-pi/aldl-connected.sh exists, run it when buffered... */
    if(aldl->ready == 1 && ran_connected_script == 0) {
      if(access("/etc/aldl-pi/aldl-connected.sh",X_OK) != -1) {
        system("/etc/aldl-pi/aldl-connected.sh");
      }
    ...
    if(ran_connected_script == 1 && get_connstate(aldl) > 10) {
      if(access("/etc/aldl-pi/aldl-disconnected.sh",X_OK) != -1) {
        system("/etc/aldl-pi/aldl-disconnected.sh");
      }
